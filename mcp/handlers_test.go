...
func TestHandleListTools(t *testing.T) {
	...
	if first.InputSchema == nil {
		t.Error("expected input schema to be populated")
	}
}
...
func TestHandleCallToolWithMCPError(t *testing.T) {
	...
	if callResult.Content[0].Text != "Intentional test error" {
		t.Errorf("Expected error message in content, got %q", callResult.Content[0].Text)
	}
}
