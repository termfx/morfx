...
func TestProviderMetadata(t *testing.T) {
	...
	// Test Lang()
	if got := p.Lang(); got != "javascript" {
		t.Errorf("Lang() = %q, want %q", got, "javascript")
	}

	// Test Aliases()
	aliases := p.Aliases()
	expectedAliases := []string{"javascript", "js", "node", "nodejs"}
	...
	expectedExtensions := []string{".js", ".mjs", ".cjs", ".jsx"}
...
func TestGetNodeKind(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.nodeType, func(t *testing.T) {
			...
			if _, ok := p.(*JavaScriptProvider); !ok {
				t.Error("Provider is not a *JavaScriptProvider")
			}
		})
	}
}
...
func TestProviderWithActualCode(t *testing.T) {
	...
	// Test with actual JavaScript code
	code := []byte(`
function main() {
	console.log("Hello");
}
