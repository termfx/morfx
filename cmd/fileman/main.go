...
func main() {
	...
	var (
		...
		failIfNoMatch = flag.Bool("fail-if-no-match", false, "Exit with an error code if no matches are found across all files.")
	...
	flag.Var(&inputFiles, "file", "File(s) to process (can be specified multiple times). Use '-' for stdin.")
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: %s [options]\n\n", os.Args[0])
		fmt.Fprintln(os.Stderr, "A powerful tool for file content manipulation based on regular expressions.")
