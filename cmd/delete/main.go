...
const deleteHelp = `Usage: delete [-h]
...
func main() {
	...
	req, err := toolenv.ReadJSON[deleteRequest](os.Stdin)
	if err != nil {
	...
	if result.Error != nil {
		_ = toolenv.WriteError(os.Stdout, "delete operation failed", result.Error)
		os.Exit(1)
	}
	...
	responseText := formatDeleteResponse(result, src.Path, src.FromFile, wroteFile)
...
func formatDeleteResponse(result core.TransformResult, path string, fromFile bool, applied bool) string {
	var builder strings.Builder
	builder.WriteString("✅ Delete operation completed successfully\n\n")
	...
	builder.WriteString(fmt.Sprintf("Deletions made: %d\n", result.MatchCount))
