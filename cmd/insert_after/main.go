...
const insertAfterHelp = `Usage: insert_after [-h]
...
Input schema:
{
  ...
  "content":  "<text to insert after matches>"
}
...
func main() {
	...
	req, err := toolenv.ReadJSON[insertAfterRequest](os.Stdin)
	if err != nil {
	...
	if result.Error != nil {
		_ = toolenv.WriteError(os.Stdout, "insert_after operation failed", result.Error)
		os.Exit(1)
	}
