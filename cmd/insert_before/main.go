...
const insertBeforeHelp = `Usage: insert_before [-h]
...
Input schema:
{
  ...
  "content":  "<text to insert before matches>"
}
Exactly one of "source" or "path" must be provided. When "path" is provided
and the transformation succeeds the file is updated in place.
...
type insertBeforeRequest struct {
...
func main() {
	...
	req, err := toolenv.ReadJSON[insertBeforeRequest](os.Stdin)
	if err != nil {
	...
	if result.Error != nil {
		_ = toolenv.WriteError(os.Stdout, "insert_before operation failed", result.Error)
		os.Exit(1)
	}
	...
	responseText := formatInsertResponse("Insert before", result, req.Content, src.Path, src.FromFile, wroteFile)
...
func formatInsertResponse(label string, result core.TransformResult, content string, path string, fromFile bool, applied bool) string {
	var builder strings.Builder
	builder.WriteString("✅ ")
	builder.WriteString(label)
	builder.WriteString(" operation completed successfully\n\n")
	...
	builder.WriteString(fmt.Sprintf("Insertions made: %d\n", result.MatchCount))
	builder.WriteString("\nInserted content:\n")
