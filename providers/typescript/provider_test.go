...
func TestTypeScriptProvider_Transform_Append_ClassScope(t *testing.T) {
	provider := New()
...
`
	...
	if result.Modified != expected {
		t.Fatalf("Unexpected append placement.\nExpected:\n%s\nGot:\n%s", expected, result.Modified)
	}
}

func TestTypeScriptProvider_Transform_Append_ImplicitClassScope(t *testing.T) {
	provider := New()
	source := `export class UserService {
  findAll() {
    return [];
...
`
	...
	if result.Modified != expected {
		t.Fatalf("Unexpected implicit append placement.\nExpected:\n%s\nGot:\n%s", expected, result.Modified)
	}
}
